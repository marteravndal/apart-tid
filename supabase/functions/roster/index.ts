...
const appUrl=()=>Deno.env.get("APP_URL")||"https://tid.apartstavanger.no";
...
Deno.serve(async(req:Request)=>{
  ...
  if(req.method==="GET"){
    ...
    const [{data:settings,error:settingsError},{data:staff,error:staffError},{data:shifts,error:shiftError},{data:vacations,error:vacationError},{data:sickLeave,error:sickError}]=await Promise.all([
  ...
  if(req.method==="POST"&&action==="save_shift"){
    ...
    const {data:employee}=await admin.from("employees").select("id,full_name").eq("id",employeeId).eq("organization_id",me.organization_id).eq("active",true).maybeSingle();if(!employee)return json({error:"Den ansatte er ikke aktiv."},400);
    const [{data:vacation},{data:sickLeave}]=await Promise.all([
      admin.from("vacation_requests").select("id,request_type").eq("organization_id",me.organization_id).eq("employee_id",employeeId).eq("status","approved").lte("start_date",workDate).gte("end_date",workDate).limit(1).maybeSingle(),
      admin.from("sick_leave_requests").select("id,absence_type,status").eq("organization_id",me.organization_id).eq("employee_id",employeeId).neq("status","rejected").lte("start_date",workDate).gte("end_date",workDate).limit(1).maybeSingle()
    ]);
    if(vacation)return json({error:`${employee.full_name} har registrert ${vacation.request_type==="vacation"?"ferie":"permisjon"} denne dagen.`},409);
    if(sickLeave)return json({error:`${employee.full_name} har registrert sykefravær denne dagen.`},409);
    ...
    const {data:before}=id?await admin.from("scheduled_shifts").select("id,employee_id,work_date,shift_type,start_time,end_time").eq("id",id).eq("schedule_id",schedule.id).maybeSingle():{data:null};
    const payload={schedule_id:schedule.id,organization_id:me.organization_id,employee_id:employeeId,work_date:workDate,shift_type:type,start_time:start,end_time:end,created_by:authData.user.id,updated_at:new Date().toISOString()};const result=id?await admin.from("scheduled_shifts").update(payload).eq("id",id).eq("schedule_id",schedule.id).select("id").maybeSingle():await admin.from("scheduled_shifts").insert(payload).select("id").single();if(result.error||!result.data)return json({error:result.error?.message||"Vakten finnes ikke."},400);await admin.from("shift_schedules").update({updated_at:new Date().toISOString()}).eq("id",schedule.id);await admin.from("audit_logs").insert({organization_id:me.organization_id,actor_id:authData.user.id,action:id?"update_roster_shift":"create_roster_shift",entity_type:"scheduled_shift",entity_id:result.data.id,details:{reason:String(body.reason||"").trim()||null,before,after:{employee_id:employeeId,work_date:workDate,shift_type:type,start_time:start,end_time:end}}});return json({id:result.data.id},id?200:201);
  }
  if(req.method==="DELETE"&&action==="delete_shift"){
    const schedule=await getSchedule(false);if(!schedule)return json({ok:true});const id=String(body.id||""),{data:before}=await admin.from("scheduled_shifts").select("id,employee_id,work_date,shift_type,start_time,end_time").eq("id",id).eq("schedule_id",schedule.id).maybeSingle();const {error}=await admin.from("scheduled_shifts").delete().eq("id",id).eq("schedule_id",schedule.id);if(error)return json({error:error.message},400);await admin.from("shift_schedules").update({updated_at:new Date().toISOString()}).eq("id",schedule.id);await admin.from("audit_logs").insert({organization_id:me.organization_id,actor_id:authData.user.id,action:"delete_roster_shift",entity_type:"scheduled_shift",entity_id:id,details:{before}});return json({ok:true});
  }
